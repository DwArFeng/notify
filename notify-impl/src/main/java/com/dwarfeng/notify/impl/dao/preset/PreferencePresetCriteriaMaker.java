...
@Component
public class PreferencePresetCriteriaMaker implements PresetCriteriaMaker {

    @Override
    public void makeCriteria(DetachedCriteria detachedCriteria, String s, Object[] objects) {
        switch (s) {
            case PreferenceMaintainService.CHILD_FOR_NOTIFY_SETTING:
                childForNotifySetting(detachedCriteria, objects);
                break;
            case PreferenceMaintainService.CHILD_FOR_TOPIC:
                childForTopic(detachedCriteria, objects);
                break;
            case PreferenceMaintainService.CHILD_FOR_USER:
                childForUser(detachedCriteria, objects);
                break;
            case PreferenceMaintainService.CHILD_FOR_NOTIFY_SETTING_TOPIC_USER_PREFERENCE_ID_ASC:
                childForNotifySettingTopicUserPreferenceIdAsc(detachedCriteria, objects);
                break;
            default:
    ...
    @SuppressWarnings("DuplicatedCode")
    private void childForNotifySettingTopicUserPreferenceIdAsc(DetachedCriteria detachedCriteria, Object[] objects) {
        try {
            if (Objects.isNull(objects[0])) {
                detachedCriteria.add(Restrictions.isNull("notifySettingId"));
            } else {
