...
@Component
public class SenderSupportPresetCriteriaMaker implements PresetCriteriaMaker<HibernateSenderSupport> {

    @Override
    public void makeCriteria(
            CriteriaBuilder criteriaBuilder, CriteriaQuery<?> criteriaQuery, Root<HibernateSenderSupport> root,
            String s, Object[] objects
    ) {
        switch (s) {
            case SenderSupportMaintainService.ID_LIKE:
                idLike(criteriaBuilder, criteriaQuery, root, objects);
                break;
            case SenderSupportMaintainService.LABEL_LIKE:
