...
public class VariablePresetCriteriaMaker implements PresetCriteriaMaker {
    ...
    public void makeCriteria(DetachedCriteria detachedCriteria, String s, Object[] objects) {
        switch (s) {
            case VariableMaintainService.CHILD_FOR_NOTIFY_SETTING:
                childForNotifySetting(detachedCriteria, objects);
                break;
            case VariableMaintainService.CHILD_FOR_TOPIC:
                childForTopic(detachedCriteria, objects);
                break;
            case VariableMaintainService.CHILD_FOR_USER:
                childForUser(detachedCriteria, objects);
                break;
            case VariableMaintainService.CHILD_FOR_NOTIFY_SETTING_TOPIC_USER_VARIABLE_ID_ASC:
                childForNotifySettingTopicUserVariableIdAsc(detachedCriteria, objects);
                break;
            default:
    ...
    @SuppressWarnings("DuplicatedCode")
    private void childForNotifySettingTopicUserVariableIdAsc(DetachedCriteria detachedCriteria, Object[] objects) {
        try {
            if (Objects.isNull(objects[0])) {
                detachedCriteria.add(Restrictions.isNull("notifySettingId"));
            } else {
